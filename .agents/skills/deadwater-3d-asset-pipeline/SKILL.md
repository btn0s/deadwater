...
Resolve `<this-skill-dir>` in the commands below in this order: `~/.claude/skills/deadwater-3d-asset-pipeline`, `~/.codex/skills/deadwater-3d-asset-pipeline`, `~/.agents/skills/deadwater-3d-asset-pipeline`, or repo `skills/deadwater-3d-asset-pipeline`.
...
`TRIPO_API_KEY=MISSING` is only a valid skip/blocker reason when this output is shown. Keys defined only in a shell profile can be absent from the process env; if the plain probe prints MISSING unexpectedly, wrap it: `zsh -lc 'source ~/.zprofile 2>/dev/null || true; source ~/.zshrc 2>/dev/null || true; python3 <this-skill-dir>/scripts/threejs_3d_asset.py probe'`. When the director skill is loaded, prefer `deadwater-game-director/scripts/probe_asset_credentials.sh`, which probes all three asset keys at once.
...
- Load `references/image-generator-workflows.md` before pairing `deadwater-image-generator` with this skill for 2D concepts, texture references, UI art, logos, decals, or image-to-3D inputs.
...
Image to 3D from a local `deadwater-image-generator` concept:
...
Use `deadwater-image-generator` before 3D generation when the asset benefits from a strong 2D reference:
...
- Use generated 3D assets as hero/high-fidelity content, then build surrounding prop kits procedurally or with additional `deadwater-3d-asset-pipeline` / `deadwater-image-generator` passes.
