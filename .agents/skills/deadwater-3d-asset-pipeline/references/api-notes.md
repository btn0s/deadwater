...
- Before rigging, check `rendered_image` actually shows a T/A-pose. If not, regenerate with a stronger pose instruction or a `deadwater-image-generator` T-pose reference image instead of rigging a bad pose.
