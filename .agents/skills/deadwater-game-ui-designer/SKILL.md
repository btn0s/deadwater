...
Load `deadwater-image-generator` when logos, icons, GUI art, faction marks, menu backgrounds, decals, or 2D HUD assets would improve quality. Use `deadwater-3d-asset-pipeline` only for 3D menu/showcase objects or diegetic 3D UI props, not normal flat HUD elements.
