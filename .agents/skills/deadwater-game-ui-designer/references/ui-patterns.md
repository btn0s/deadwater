...
Use `deadwater-image-generator` for 2D interface assets when hand-coded CSS/icons are not enough:
...
Use `deadwater-3d-asset-pipeline` only when the UI needs a 3D object: rotating character preview, vehicle garage, weapon inspect model, trophy, diorama, or diegetic 3D menu prop.
...
- Generated 2D asset path and prompt when `deadwater-image-generator` was used.
- Imported 3D preview model path and renderer diagnostics when `deadwater-3d-asset-pipeline` was used.
